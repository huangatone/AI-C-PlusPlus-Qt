...
void DFSplitterHandle::paintEvent(QPaintEvent *event)
{
	...
	for( i=1 ; i< splitter()->count(); i++)
	  ...
	  if (orientation() == Qt::Horizontal)
	  {
		  ...
		  icon = QRect( 1, h_rt.height()/2 -8, h_rt.width()-2, 16);
		  l = QLine( QPoint(h_rt.width()/2,h_rt.height()/2 -8), QPoint(h_rt.width()/2,h_rt.height()/2 + 8));
	  }
	  else
	  {		 
	  ...
	  if(fr_rt.contains( pt1) )
	   ...
	   //painter.drawPixmap(icon ,pix, pix.rect());
	  // painter.fillPath(tri_path,QBrush( Qt::black));
	  QPolygon pl;
