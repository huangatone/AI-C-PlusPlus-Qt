...
TextAreaDetect::TextAreaDetect()
{

}


const string IMAGEPATH = "/home/parallels/devel/mine/tmp/demo.jpeg";//图像路径


Mat TextAreaDetect::preprocess(Mat gray)
{
	//1.Sobel算子，x方向求梯度
	Mat sobel;
	...
	//7.存储中间图片
    imwrite("/home/parallels/devel/mine/tmp/dbinary.jpg", binary);
    imwrite("/home/parallels/devel/mine/tmp/ddilate1.jpg", dilate1);
    imwrite("/home/parallels/devel/mine/tmp/derode1.jpg", erode1);
    imwrite("/home/parallels/devel/mine/tmp/ddilate2.jpg", dilate2);

	return dilate2;
}


vector<RotatedRect> TextAreaDetect::findTextRegion(Mat img)
{
...
void TextAreaDetect::detect(Mat img)
{
	//1.转化成灰度图
	Mat gray;
	...
	//4.用绿线画出这些找到的轮廓
	foreach (RotatedRect rect , rects)
	{
		Point2f P[4];
		rect.points(P);
		for (int j = 0; j <= 3; j++)
	...
	//5.显示带轮廓的图像
	imshow("img", img);
    imwrite("/home/parallels/devel/mine/tmp/dimgDrawRect.jpg", img);

	waitKey(0);
}


int TextAreaDetect::work()
{
